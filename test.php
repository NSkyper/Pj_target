...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($year && $branch_id) {
        ...
        try {
            ...
            $delete_sql = "DELETE FROM target WHERE target_year = :year AND branch_id = :branch_id";
            ...
            for ($month = 1; $month <= 12; $month++) {
                ...
                $insert_sql = "INSERT INTO target (target_year, target_month, branch_id, target_amt) 
                               VALUES (:year, :month, :branch_id, :target_amt) 
                               ON DUPLICATE KEY UPDATE target_amt = VALUES(target_amt)";
                $stmt = $conn->prepare($insert_sql);
                $stmt->bindParam(':year', $year, PDO::PARAM_INT);
                $stmt->bindParam(':month', $month, PDO::PARAM_INT);
                $stmt->bindParam(':branch_id', $branch_id, PDO::PARAM_INT);
                $stmt->bindParam(':target_amt', $monthly_target, PDO::PARAM_INT);
                if (!$stmt->execute()) {
            ...
            if ($month_count > 0) {
                ...
                for ($month = 1; $month <= 12; $month++) {
                    ...
                    if ($current_target <= 0) {
                        ...
                        $stmt->bindParam(':month', $month, PDO::PARAM_INT);
                        $stmt->bindParam(':branch_id', $branch_id, PDO::PARAM_INT);
                        $stmt->bindParam(':target_amt', $average_target, PDO::PARAM_INT);
                        if (!$stmt->execute()) {
                            throw new Exception("Execute failed: " . implode(" ", $stmt->errorInfo()));
                        }
                    }
...
function getBranches($conn, $areazone_id = null)
{
    ...
    if ($areazone_id) {
        $sql .= " AND areazone_id = :areazone_id";
    }
    $stmt = $conn->prepare($sql);
    if ($areazone_id) {
...
// Query to fetch targets for display
...
<body>
    ...
    <table border="1" id="data-display" class="step">
        ...
        <tbody>
            <?php while($row = $result(PDO::FETCH_ASSOC)): ?>
            <tr>
